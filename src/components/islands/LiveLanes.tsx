...
function Row({ row, buySym }: { row: LaneRow; buySym: string }) {
  ...
  return (
    <li
      className={cn(
        "flex items-center gap-2.5 border-2 px-3 py-2.5 text-[13px]",
        row.isBest ? "border-accent bg-accent-100" : "border-divider",
      )}
    >
      {lane && (
        <img
      ...
      {row.isBest && (
        <span className="bg-accent px-1.5 py-0.5 text-[11px] uppercase tracking-[0.06em] text-ground">
          best
        </span>
      )}
      <span className={row.isBest ? "font-heading font-extrabold" : ""}>
        {railLabel(row.rail, row.hops)}
      </span>
      {lane && (
      ...
      {!row.isBest && (
        <span className="text-ink/55 tnum">{row.deltaBps} bps</span>
      )}
      <span className="tnum">
...
export default function LiveLanes({ sym }: { sym: string }) {
  ...
  if (shouldFallback(status, rows.length)) {
    return (
      <div className="border-2 border-ink bg-ground p-5">
        <p className="m-0 mb-3 text-[13px] text-ink/70">
          We route {sym} across every venue.
        </p>
  ...
  const spreadBps =
    liveCount > 1 ? Math.abs(rows[rows.length - 1].deltaBps) : null;
    ...
    <div className="border-2 border-ink bg-ground p-5">
      <div className="mb-3 flex items-center gap-2.5">
        ...
        <div className="leading-tight">
          <p className="m-0 font-heading font-extrabold text-[15px]">
            Routing $1,000 into {sym}
          </p>
          <p className="m-0 text-[12px] text-ink/55">
            {status === "loading"
              ? "quoting…"
              : `${liveCount} of ${LANES.length} venues quoting${spreadBps !== null ? ` · best beats worst by ${spreadBps} bps` : ""}`}
          </p>
        </div>
      </div>
      {status === "loading" && liveCount === 0 ? (
        <ul className="m-0 grid list-none gap-2 p-0">
          {[0, 1, 2].map((i) => (
            <li
              key={i}
              className="h-[42px] animate-pulse border-2 border-divider bg-surface"
            />
          ))}
        </ul>
      ) : (
        <ul className="m-0 grid list-none gap-2 p-0">
          {rows.map((r) => (
            <Row key={r.rail} row={r} buySym={buySym} />
          ))}
        </ul>
      )}
      <div className="mt-4 border-t-2 border-divider pt-3">
        <p className="m-0 mb-2 text-[12px] text-ink/55">
          We route across every venue
        </p>
        <LaneList variant="strip" />
      </div>
    </div>
