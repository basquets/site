...
function q(rails: { rail: string; buyAmount: string }[], best: string): ApiQuote {
  return {
    sell: { address: "0x", symbol: "USDG", decimals: 6, amount: "1000000000" },
    buy: { address: "0x", symbol: "NVDA", decimals: 18 },
    mid: null, impact: null, depth: null,
    rails: rails.map((r) => ({ ...r, gasEstimate: null, hops: null, tx: null })),
...
describe("laneRows", () => {
  ...
  test("spread: worst row delta is negative", () => {
    const rows = laneRows(q(
      [{ rail: "v4", buyAmount: "4800200000000000000" }, { rail: "lifi", buyAmount: "4788200000000000000" }],
      "v4",
    ));
